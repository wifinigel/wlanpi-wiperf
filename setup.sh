...
install () {
  ...
  echo "(ok) Version: ${VERSION}, branch: ${GITHUB_BRANCH})" | tee -a $LOG_FILE
  ...
  python3 wiperf2/setup.py install >> $LOG_FILE 2>&1
   if [ "$?" != '0' ]; then
...
# Check which platform we're installing for
  ...
  curl -s https://raw.githubusercontent.com/wifinigel/wiperf2/${GITHUB_BRANCH}/setup.sh | sudo bash -s install $PLATFORM
...
check_ver () {
  ...
  current_ver=$VERSION
  available_ver="`wget -qO- --timeout=1 --tries=2 https://raw.githubusercontent.com/wifinigel/wiperf2/main/version.txt`"
